    ...
    MIGRATION_1_2 {
        override fun build()= object : Migration(1, 2) {
            override fun migrate(database: SupportSQLiteDatabase) {
                with(database){
                    execSQL("CREATE TABLE IF NOT EXISTS `contatos_tb_bkp` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `nome_contatos` TEXT NOT NULL, `email_contatos` TEXT NOT NULL, `proprietario` TEXT NOT NULL, `placa` TEXT NOT NULL, `renavam` TEXT NOT NULL)")
                    ...
                    execSQL("INSERT INTO contatos_tb_bkp (id, nome_contatos, email_contatos, proprietario, placa, renavam) SELECT id, nome_contatos, email_contatos, proprietario, placa, renavam FROM contatos_tb")
